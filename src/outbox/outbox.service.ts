...
export class OutboxService {
  ...
  public async poll(): Promise<void> {
    setInterval(() => {
      console.log("----- Polling Outbox table from Order DB -------");
      return this.findUnsentEvents();
    }, this.pollingInterval);
  }

  async findUnsentEvents() {
    const unSent = await this.outboxRepository.findAll<Outbox>({ where: { sent: 0 } });
    ...
    unSent.forEach((e: Outbox) => {
      if (e.getDataValue("type") === PENDING) {
        ...
        sent++;
        console.log(`---------- Sent ${sent}/${count} ---------`);
        this.outboxRepository.update({ sent: 1 }, { where: { id: e.getDataValue("id") } });
      }
    });
